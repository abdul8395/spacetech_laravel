...
class Pss extends connection
{
    ...
    public function get_data()
    {
        ...
        $sql="with foo as (select ST_Transform(st_setsrid(ST_GeomFromGeoJSON ('$geom'),4326),32643)as geom)
                ,bar as (
                select a.gid,site_name as site_name, round((st_distance(st_transform(a.geom,32643),foo.geom)/1000)::numeric,2) as distance  from water.tbl_existing_schemes a,foo
                ...
                ORDER BY foo.geom <-> st_transform(a.geom,32643) limit 1
                )select (select distance as distance_from_site from bar ),(select site_name  from bar ),*,
