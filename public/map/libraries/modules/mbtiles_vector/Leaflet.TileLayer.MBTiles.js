...
L.TileLayer.MBTilesVector = L.TileLayer.extend({
	...
	_openDB: function(buffer) {
		try {
			...
			this._stmt = this._db.prepare('SELECT tile_data FROM map inner join images '+
				'on map.tile_id = images.tile_id WHERE zoom_level = :z AND tile_column = :x AND tile_row = :y');
...
*/
L.tileLayer.mbTilesVector = function(databaseUrl, options) {
	return new L.TileLayer.MBTilesVector(databaseUrl, options);
}
