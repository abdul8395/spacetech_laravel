...
class Pss extends connection
{
    ...
    public function loadData()
    {
       ...
       $geom1=$_REQUEST['geom'];
        ...
        $sql1="select count(*) as total_schools,sum(enrollment) as school_going FROM education.schools_data_with_locations WHERE ST_Within(geom, st_geomfromtext(st_astext(ST_GeomFromGeoJSON('$geom1')),4326))";
        $res1 = pg_query($sql1);
        $val2 = pg_fetch_all($res1);

        $sql2 = "with foo as (select ST_Transform(st_setsrid(ST_GeomFromGeoJSON ('$geom1'),4326),32643)as geom)
